    ...
    return (
        <article className={styles.recenzijeHomeCoverContainer}>
            ...
            <section className={styles.coverContent}>
                <Link href={`/recenzije?page=1&sort=createdAt&order=desc`}>
                    <h3>Recenzije</h3>
                </Link>
                <StandardBtn type='right' path={`/recenzije?page=1&sort=createdAt&order=desc`} content='Pogledaj sve' span={`(${count.numberOfMovies})`} newTab={false}/>
            </section>
        </article>
    )
}